...
\section{The fundamental sequence}
    ...
    \begin{definition}
        The torsion is called a \blue{shaggy torsion} if $\mu \in \pir$. Otherwise, in case of $\pi$-irrational torsion $\mu \in \pirr$, it is called a \blue{sleek torsion}. 
    \end{definition} 
    ...
    \begin{theorem}
        ...
        $\mu = \frac{2\pi M}{N}$ for some integers $M \in \mathbb Z,\, N \in \mathbb N$ and  $\gcd(M,N)=1$, then the brim and the frill each consist of roots all of the polynomials$$
    ...
    \end{theorem}
    \begin{proof}
        This is plausible.%, e.g. see the next theorem (Theorem~\ref{th:intbrim}).
    \end{proof}
    ...
    \begin{proof}
        ...
        Since the torsion is sleek, one can always choose arbitrarily large $N$ and yet $N\mu/pi$ close enough to an integer that \be\label{eq:appint}
            \sum_{n=M}^{M+N}\ln\rho_{ r_n} = \sum_{n=M}^{M+N}\ln|s + 2t \sin\mu| = \int_0^{2\pi} \ln |s + 2t \sin\mu| d\mu + \frac{2\pi\xi\delta(s,t)}N  = I +  \frac{2\pi\xi\delta(s,t)}N,
        \ee where $0 \le \mu_n = \mu n - 2\pi k < 2 \pi,\, -1\le\xi\le1 $ and  $$
            \delta(s,t)= \max_{\mu}\left| \frac{\df{\rho_{ r_1}}}{\df{\mu}} \right|,
        $$  because ${\rho_{ r_1}}$ is a smooth function of $\mu$.
        The equation (\ref{eq:appint}) written with $M=k*N$ for $k<L$ implies $$
            2l>\left|\sum_{n=N}^{LN}\ln\rho_{ r_n}\right| >\left|\frac L 2 I \right|,
        $$
        because one can always keep the relation $L/N$ smaller than $I$ for arbitrarily chosen $L$.   
...
\section{The elemental roots}
    \begin{definition}
        ...
        ,$$ called \blue{petals}, such that $$
            ...
            \; \exists k\le N %\in \mathbb N
            ,\,w %\in \mathbb R
            :\; a_n = f_k(w) 
        $$ and $$
            \forall w,\,\varepsilon >0,\,k\le N \; \exists n :\; |a_n - f_k(w)|<\varepsilon
            %\in \mathbb R,\,k\le N \in \mathbb N,\, \forall \varepsilon >0  \in \mathbb R \; \exists n \in \mathbb N: |a_n - f_k(w)|<\varepsilon
        .$$
    ...
    \begin{theorem}
        For a fancy frond and a sleek torsion the angle $$
            \varphi_s = \frac{2\pi M}N, \qquad M \in \mathbb Z,\, N \in\mathbb N 
        $$ will be an elemental root of merosity $N$ if $\gcd(M,N)=1$.
    \end{theorem}
    \begin{proof}
        \red{Needs to be written.}
    \end{proof}
